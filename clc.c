...
static void pretty_print_answer(evaluation_result result)
{
	...
	if ((strcmp(buffer, "inf") != 0 && strcmp(buffer, "-inf") != 0) &&
		...
		if (-3 <= mantissa && mantissa <= num_decimal_places_e_form) {
			...
			// 3-10
			if ((8 <= number_of_nines && number_of_nines <= num_decimal_places-3) || (nines_result.d_count == 4 && num_decimal_places >= 15)) {
			...
			// 9-12
			if ((9 <= number_of_zeros && number_of_zeros <= num_decimal_places-3) || (zeros_result.d_count >= 4 && num_decimal_places >= 15)) {
